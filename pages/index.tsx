      ...
      <main className="">
        <div className="text-center p-2 pb-6 mb-5">
          <img
            src="icon.jpg"
            alt=""
            className="w-24 h-24 m-auto p-3"
          />
          <h1 className="text-lg mt-5">世界の裏側へようこそ</h1>
          <br/>
          <p className="">ここでは、知るきっかけを提供することを目的とし運営しています。</p>
        </div>

        <h2 className="bg-pink-400 p-1 text-center rounded-full text-white mb-1">カテゴリ：生活</h2>
        <div className="flex ml-1 overflow-x-scroll">
          <div className="whitespace-nowrap h-56 w-60 sm:h-96 sm:w-full">
            {allPostsData
            .filter((data) => { return data.category === "生活" })
        ...
        <h2 className="bg-gray-600 p-1 text-center rounded-full text-white mb-1">カテゴリ：経済</h2>
        <div className="flex ml-1 overflow-x-scroll">
          <div className="whitespace-nowrap h-56 w-60 sm:h-96 sm:w-full">
            {allPostsData
            .filter((data) => { return data.category === "経済" })
            .map(({ pageTitle, title, date, img, description }) => (
              <section
              className="border rounded-md border-gray-400 mb-2 px-1 w-60 sm:w-96
            hover:border-gray-300 cursor-pointer mr-1 inline-block whitespace-normal"
        ...
        <h2 className="bg-blue-400 p-1 text-center rounded-full text-white mb-1">カテゴリ：教育</h2>
        <div className="flex ml-1 overflow-x-scroll">
          <div className="whitespace-nowrap h-56 w-60 sm:h-96 sm:w-full">
            {allPostsData
            .filter((data) => { return data.category === "教育" })
            .map(({ pageTitle, title, date, img, description }) => (
              <section
              className="border rounded-md border-gray-400 mb-2 px-1 w-60 sm:w-96
            hover:border-gray-300 cursor-pointer mr-1 inline-block whitespace-normal"
        ...
        <h2 className="pt-5 pl-2 text-lg border-b m-1 mb-2 border-gray-300">最新記事</h2>
        <div>
          {allPostsData
          .slice(0,3)
          .map(({ pageTitle, title, date, img, description }) => (
        ...
        <h2 className="pt-6 pl-2 text-lg border-b m-1 mb-2 border-gray-300">こちらの記事は読まれましたか？</h2>
        <div className="">
          <div className="">
            {allPostsData
            .slice(4, 5)
